...
public class TextBuilder {
     ...
     */
    private static final String[] DEFAULT_SEPARATORS_SEQUENCES = { " " };
	...
	for (final String word : words) {
		...
		m_builder.append(lineLength + word.length() + 1 <= intentedMaxLine ? " " : indentedLineSeparator());
	...
	for (final String word : words) {
	    ...
	    if (lineLength != 0)
		// Add space if needed, or line separator if went over max line
		m_builder.append(lineLength + word.length() + 1 <= m_maxLine ? " " : LINE_SEPARATOR);
	    m_builder.append(word);
	}
    }
